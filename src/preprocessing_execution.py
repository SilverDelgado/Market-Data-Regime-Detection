...
# --- Asegurar que el directorio 'src' esté en el path para importaciones ---
PROJECT_ROOT = Path(__file__).resolve().parents[1]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

# --- Importar lógica de los scripts de preprocesamiento ---
# (Asumimos que las funciones principales están en estos archivos)
...
TRAIN_SET_RATIO = 0.6  # Usar el 60% de los datos para aprender los transformadores
...
def main():
    ...
    # --- PASO 1: Generar el dataset de indicadores ---
    print(f"\n[PASO 1/3] Generando dataset de indicadores desde: {RAW_DATA_PATH.name}")
    try:
        # Asumimos que la función de preprocesamiento devuelve la ruta del archivo creado
    ...
    # --- PASO 2: Aprender y guardar los artefactos del pipeline ---
    ...
    # --- PASO 3: Aplicar los artefactos para crear el dataset final ---
    print(f"\n[PASO 3/3] Aplicando transformaciones a todo el dataset para crear el archivo final...")
    try:
        # Cargar los artefactos recién guardados
        pipeline_artefacts = joblib.load(ARTEFACTS_PATH)
        
        # Aplicar la transformación al dataset completo de indicadores
        df_final = apply_pipeline_transformations(df_indicators, pipeline_artefacts)
        
        # Guardar el archivo final, listo para experiment_hmm.py
        df_final.to_csv(FINAL_DATA_PATH, index=True) # index=True para guardar la columna datetime
